...
export const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, inventoryItems }) => {
  ...
  const getFilteredItems = (type: string): any[] => {
    switch (type) {
      case 'total_items':
      ...
      case 'low_stock':
        return inventoryItems
          .filter(item => item.remaining_stock <= 10 && item.remaining_stock > 0)
          .map(item => ({
            'Description': item.description,
            'Category': item.category,
            'Current Stock': item.remaining_stock,
            ...
            'Suggested Reorder': Math.max(item.sales_weekly * 2, 20),
            'Priority': item.remaining_stock <= 5 ? 'High' : 'Medium'
          }));
      ...
      case 'shopping_list':
        return inventoryItems
          .filter(item => item.remaining_stock <= 10)
          .map(item => ({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      selectedOptions.forEach(option => {
        const data = getFilteredItems(option.id);
        if (data.length > 0) {
      ...
      if (exportFormat === 'xlsx') {
        ...
        const data = getFilteredItems(firstOption.id);
        const worksheet = XLSX.utils.json_to_sheet(data);
